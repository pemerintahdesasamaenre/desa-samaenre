...
-- 2. Insert Village Info
INSERT INTO public.village_info (id, name, vision, mission, history, contact_info) VALUES
...
DECLARE
    ...
    cat_job_id UUID;
BEGIN
    SELECT id INTO cat_pop_id FROM public.categories WHERE slug = 'populasi';
    SELECT id INTO cat_dusun_id FROM public.categories WHERE slug = 'dusun';
    SELECT id INTO cat_job_id FROM public.categories WHERE slug = 'pekerjaan';
...
-- 4. SINKRONISASI USER AUTH KE PROFILES (Kritikal untuk akses Admin)
INSERT INTO public.profiles (id, full_name, role)
SELECT id, COALESCE(raw_user_meta_data->>'full_name', 'Admin Desa'), 'admin'
FROM auth.users
